...
public class FileServiceByMemoary extends FileServiceAbstractDetails {
	...
	private Bitmap getBitmapByCache(String url, ImageView imageView, Bitmap defaultBitmap, Integer scrollState) {
		resetPurgeTimer();
		// scrollState 0:停止状�?，需要加载图�?cache -> network)
		// scrollState 1/2:滚动状�?，此时就用cache图片，如果cache没有就用默认defaultBitmap
		Bitmap bitmap = getBitmapFromCache(url);
		if (scrollState != SCROLL_STATUS_STOP) {// 处于滚动状�?
			if (bitmap == null) {
	...
	/*
	 * SoftReference 指到的对象，即使没有任何 Direct Reference，也不会被清除�? �?��要到 JVM 内存不足时且 没有
	 * Direct Reference 时才会清除，SoftReference 是用来设�?object-cache 之用的�? 不但可以把对�?
	 * cache 起来，也不会造成内存不足的错�?（OutOfMemoryError�?
	 */
