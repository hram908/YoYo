...
public class DownloadEntity implements Runnable, AConstDefine, Parcelable {
	...
	public long currentPosition; // 当前下载进度�?	
	public long fileLength; // apk文件总大�?	
	public String packageName; // 包名
	public int versionCode; // 版本�?	
	private int status; // 下载状�?�?	
	...
	public String installedVersionName; // 已安装的软件版本�?	
	public long installedFileLength; // 已安装的软件大小
	public Drawable installedIcon; // 已安装的软件图标
	public int installedVersionCode; // 已安装的软件版本�?	
	public int heavy;// 权重
	...
	private InputStream getInputStream() throws MalformedURLException, IOException {
		...
		fileLength = currentPosition + httpConnection.getContentLength(); // 获取下载文件总大�?		
		return is;
	}
	...
	private void postExecute() {
		synchronized (this) {
			if (currentPosition == fileLength) {// 下载完成
				...
				completeRenameFile();
			} else if (status == STATUS_OF_EXCEPTION || status == STATUS_OF_PAUSE || status == STATUS_OF_PAUSE_ON_TRAFFIC_LIMIT) {// 下载异常、暂停或者流量限�?				notifyDownloadChange();
...
//		return packageName.hashCode() + versionCode;
		return appName.hashCode() + versionCode;
	}
	...
	public synchronized void setStatus(int status) {
		...
		switch (status) {
		case STATUS_OF_INITIAL://初始化状�?			this.currentPosition = 0;
			this.status = status;
			break;
