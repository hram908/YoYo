...
public class DownloadUtils implements AConstDefine {
	...
	public static final int STATUS_SETTING_FLOW_INSUFFICIENT = 4; // 设置流量不足以完成此次下�?
	public static final int STATUS_NOT_SDCARD = 5; // 无SD�?
	public static final int STATUS_NOT_SETTING_MOBILE_DOWNLOAD = 6; // 未设置开启蜂窝下�?
...
//			}
				...
				if (DownloadService.mDownloadService != null) {
					if (DownloadService.mDownloadService.canUseGprsDownload()) {// 是否可以使用gprs下载
						...
						Intent intent = new Intent(
								AConstDefine.BROADCAST_ACTION_NOFLOW);// 发送无流量广播
		...
		case DJMarketUtils.STATUS_NOT_SETTING_MOBILE_DOWNLOAD:// 未设置开启蜂窝下载
			DJMarketUtils.showToast(context,
					R.string.setting_for_cellular_close);
			break;
		}
	}
	 ...
	 * @param entity
	 */
	private static void sendDownloadBroadcast(Context context,
	...
	public static void checkDownload(Context context, ApkItem apkItem) {
		...
		if (null != DownloadService.mDownloadService) {
			...
			for (; i < downloadList.size(); i++) {
				...
				if (d.appName.equals(entity.appName)
						&& d.versionCode == entity.versionCode) {
					if (d.downloadType == TYPE_OF_COMPLETE) {// 下载类型为已完成类型
						String path = DOWNLOAD_ROOT_PATH + d.hashCode()
								+ DOWNLOAD_FILE_POST_SUFFIX;
						System.out.println(" DU--checkDownload--path=" + path);
						File file = new File(path);
						if (file.exists()) {// apk文件是否存在
						// installApk(context, path);//安装apk
							Toast.makeText(context, "已经下载过了",
									Toast.LENGTH_SHORT).show();
						} else {
							Intent removeIntent = new Intent(
									BROADCAST_ACTION_REMOVE_DOWNLOAD);// 发送删除广播
	 ...
	 * @param entity
	 */
	public static void checkOneKeyDownload(Context context,
			DownloadEntity entity) {
		...
		case DJMarketUtils.STATUS_ONLY_MOBILE:
			...
			// if (context instanceof ApkDetailActivity) {
			// isPromptUser = ((ApkDetailActivity)
			// context).is3GDownloadPromptUser();
			// } else if (context instanceof DownloadActivity) {
			// isPromptUser = ((DownloadActivity)
			// context).is3GDownloadPromptUser();
			// } else {
			// isPromptUser = ((PublicActivity)
			// context).is3GDownloadPromptUser();
			// }
			if (isPromptUser) {
				if (DownloadService.mDownloadService != null) {
					if (DownloadService.mDownloadService.canUseGprsDownload()) {
						context.sendBroadcast(new Intent(
								BROADCAST_ACTION_ONEKEY_UPDATE));
					} else {
						Intent intent = new Intent(
								AConstDefine.BROADCAST_ACTION_NOFLOW);
						context.sendBroadcast(intent);
					}
				}
			} else {
	...
	/**
	 * 显示用户手机gprs提示�? * @param context
	 * 
	 * @param entity
	 */
	private static void showUseMobileGprsPromptDialog(final Context context,
		...
		if (mUseGprsDialog == null) {
			...
			mUseGprsDialog.setNeutralButton(
					...
					new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							if (DownloadService.mDownloadService != null) {
								if (DownloadService.mDownloadService
										.canUseGprsDownload()) { // 能否继续使用
																	// Gprs
																	// 流量下载
									if (entity != null) {
										sendDownloadBroadcast(context, entity);
			...
			mUseGprsDialog.setNegativeButton(
	...
	public static int isCanDownload(Context context) {
		...
		} else if (!DJMarketUtils.isWifiAvailable(context)
				&& DJMarketUtils.isMobileAvailable(context)) {
		...
		case DJMarketUtils.STATUS_ONLY_MOBILE:
			if (isPromptUser) {
				if (DownloadService.mDownloadService != null) {
					if (DownloadService.mDownloadService.canUseGprsDownload()) {
						Intent intent = new Intent(
								BROADCAST_ACTION_START_ALL_DOWNLOAD);
						context.sendBroadcast(intent);
					} else {
						Intent intent = new Intent(
								AConstDefine.BROADCAST_ACTION_NOFLOW);
	...
	private static void showUseMobileGprsPromptDialog2(final Context context) {
		if (mUseGprsDialog2 == null) {
			...
			mUseGprsDialog2.setNeutralButton(
					context.getString(R.string.prompt_download),
					new View.OnClickListener() {
						@Override
						public void onClick(View v) {
			...
			mUseGprsDialog2.setNegativeButton(
					context.getString(R.string.cancel), null);
		}
	...
	public static void checkCloudRestore(Context context,
			ArrayList<ApkItem> items) {
		int status = DJMarketUtils.isCanDownload(context);
		switch (status) {
		case DJMarketUtils.STATUS_NOT_NETWORK:
			DJMarketUtils.showToast(context, R.string.no_network_msg1);
		...
		case DJMarketUtils.STATUS_SDCARD_INSUFFICIENT:
			...
			break;
		case DJMarketUtils.STATUS_ONLY_MOBILE:
			// boolean isPromptUser = ((DownloadManageActivity)
			// context).is3GDownloadPromptUser();
			boolean isPromptUser = false;
			if (isPromptUser) {
				if (DownloadService.mDownloadService != null) {
					if (DownloadService.mDownloadService.canUseGprsDownload()) {
						Intent intent = new Intent(
								BROADCAST_ACTION_CLOUD_RESTORE);
						Bundle bundle = new Bundle();
	...
	private static void showUseMobileGprsPromptDialog3(final Context context,
			final ArrayList<ApkItem> items) {
		if (mUseGprsDialog == null) {
			mUseGprsDialog = new CustomNoTitleDialog(context);
			mUseGprsDialog.setMessage(R.string.cellular_download_prompt_msg);
			mUseGprsDialog.setNeutralButton(
							...
							if (DownloadService.mDownloadService != null) {
								if (DownloadService.mDownloadService
									...
									Intent intent = new Intent(
											BROADCAST_ACTION_CLOUD_RESTORE);
									Bundle bundle = new Bundle();
									bundle.putParcelableArrayList("cloudList",
											items);
									intent.putExtras(bundle);
									context.sendBroadcast(intent);
								} else {
	...
	public static boolean checkApkFile(String apkPath) {
		...
		try {
			...
			metrics.setToDefaults();// 显示相关，这里设为默�?
	 ...
	 * @param downloadList
	 */
	public static void fillUpdateAndUpdatingNotifycation(Context context,
			boolean isAdd) {
		int updateCount = 0;
		int updatingCount = 0;
		if (DownloadService.mDownloadService != null) {
	 ...
	 * @param entity
	 */
	public static void setInstallDownloadEntity(Context context,
			DownloadEntity entity) {
