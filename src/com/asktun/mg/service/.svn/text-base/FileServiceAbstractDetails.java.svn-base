...
public abstract class FileServiceAbstractDetails implements FileServiceInterface {
	...
	static {
		mOptions.inPreferredConfig = Bitmap.Config.RGB_565;// 16位位�?
		mOptions.inPurgeable = true;// BitmapFactory创建的Bitmap用于存储Pixel的内存空间在系统内存不足时可以被回收
		mOptions.inInputShareable = true;// 位图能够共享�?��指向数据源的引用，或者是进行�?��拷贝
	}
	...
	boolean cancelPotentialDownload(String url, ImageView imageview) {
		...
		if (bitmapDownloaderTask != null) {
			...
			if ((bitmapUrl == null) || (!bitmapUrl.equals(url))) {//如果同一个imageView对应的url不同，则取消旧的url对应的下载任�?
	...
	class DownloadedDrawable extends BitmapDrawable {
		private final WeakReference<BitmapDownloaderTask> bitmapDownloaderTaskReference;// 图片下载弱引�?
