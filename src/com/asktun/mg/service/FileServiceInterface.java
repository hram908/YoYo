...
public interface FileServiceInterface {
	...
	public static final int SCROLL_STATUS_STOP = 0;//无滑�?	
	public static final boolean isRemoteImage = false;//是否运行下载图片

	// 文件服务模式 ，无同步任务 ，无下载图片 ，普�?	
	public enum Mode {
	 ...
	 * path> 该方法�?常在应用的入口处调用，并且应用从打开到关闭，仅需执行�?��（执行多次也是可以的，但是没有必要）�?	 */
	public void loadFileToMap();

	/**
	 * 通过URL获取图片，首先检查本地信�?SD card，内存，data目录)，如果没有则从网络获�?	 * 
	 * @param url
	 *            图片的URL
	 * @param imageView
	 *            �?��set图片的ImageView
	 * @param defaultBitmap
	 *            如果不能通过网络或�?本地cache获取图片，则使用这张Bitmap作为ImageView的背�?	 * @param scrollState
	 *            判断ListView的滚动状态，默认是不滚动
	 */
	public void getBitmap(String url, ImageView imageView, Bitmap defaultBitmap, Integer scrollState);

	/**
	 * 通过URL获取图片，图片可能保存在 memory或�?sd card上，
	 * 注意:调用本方法之前，�?��要先调用FileService.loadFileToMap();
	 * 
	 * @param url
	 *            图片的URL[类型:String]
	 * @param imageView
	 *            �?��set图片的ImageView[类型:ImageView]
	 * @param defaultBitmap
	 *            如果不能通过网络或�?本地cache获取图片，则使用这张Bitmap作为ImageView的背景[类型:Bitmap]
	 * @param isRemote
	 *            判断是否允许获取网络图片
	 * 
	 */
